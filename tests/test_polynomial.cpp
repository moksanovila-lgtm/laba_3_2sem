...
template <typename T>
std::string PolyToStr(Polynomial<ArraySequence, T>& p) {
    return p.ToString();
}
...
protected:
    void SetUp() override {
        intPoly1 = new Polynomial<ArraySequence, int>{1, 2, 3};     
        intPoly2 = new Polynomial<ArraySequence, int>{4, 5};         
        intPoly3 = new Polynomial<ArraySequence, int>{1, 0, -1};      
        
        doublePoly = new Polynomial<ArraySequence, double>{1.5, 2.5, 3.5};
    }
    ...
    Polynomial<ArraySequence, int>* intPoly1;
    Polynomial<ArraySequence, int>* intPoly2;
    Polynomial<ArraySequence, int>* intPoly3;
    Polynomial<ArraySequence, double>* doublePoly;
};
...
TEST_F(PolynomialTest, IntAdd) {
    ...
    EXPECT_EQ(got, expected) << "Add: " << before1 << " + " << before2 << " = expected " << expected << ", got " << got;
}
...
TEST_F(PolynomialTest, IntMultiply) {
    ...
    EXPECT_EQ(got, expected) << "Multiply: " << before1 << " * " << before2 << " = expected " << expected << ", got " << got;
}
...
TEST_F(PolynomialTest, IntMultiplyByScalar) {
    ...
    EXPECT_EQ(got, expected) << "MultiplyByScalar(2): input=" << before << ", expected " << expected << ", got " << got;
}

TEST_F(PolynomialTest, IntEvaluate) {
    std::string before = PolyToStr(*intPoly1);
    int result = intPoly1->Evaluate(2);
    EXPECT_EQ(result, 17) << "Evaluate(2): input=" << before << ", expected 17, got " << result;
}
...
TEST_F(PolynomialTest, IntCompose) {
    ...
    EXPECT_EQ(got, expected) << "Compose: " << before1 << "(" << before2 << ") = expected " << expected << ", got " << got;
}
...
TEST_F(PolynomialTest, IntOperatorPlus) {
    ...
    std::string got = PolyToStr(result);
    std::string expected = "3x^2+7x+5";
    EXPECT_EQ(got, expected) << "operator+: " << before1 << " + " << before2 << " = expected " << expected << ", got " << got;
}
...
TEST_F(PolynomialTest, DoubleEvaluate) {
    ...
    EXPECT_DOUBLE_EQ(result, expected) << "Evaluate(2.0): input=" << before << ", expected " << expected << ", got " << result;
}

TEST_F(PolynomialTest, DoubleAdd) {
    Polynomial<ArraySequence, double> p1{1.0, 2.0, 3.0};
    Polynomial<ArraySequence, double> p2{4.0, 5.0};
    auto result = p1.Add(p2);
    std::string got = PolyToStr(result);
    std::string expected = "3x^2+7x+5";
    EXPECT_EQ(got, expected) << "Double Add: expected " << expected << ", got " << got;
}

TEST(PolynomialBoundaryTest, EmptyPolynomial) {
    Polynomial<ArraySequence, int> p;
...
TEST(PolynomialBoundaryTest, ZeroPolynomial) {
    Polynomial<ArraySequence, int> p{0, 0, 0};
    std::string got = p.ToString();
    std::string expected = "0";
...
TEST(PolynomialBoundaryTest, PolynomialWithZeroCoefficients) {
    Polynomial<ArraySequence, int> p{1, 0, 0, 2};
    std::string got = p.ToString();
    std::string expected = "2x^3+1";
    EXPECT_EQ(got, expected) << "Polynomial with zero coefficients: expected " << expected << ", got " << got;
    EXPECT_EQ(p.GetDegree(), 3) << "Degree should be 3, got " << p.GetDegree();
}

TEST(PolynomialBoundaryTest, NegativeCoefficients) {
    Polynomial<ArraySequence, int> p{1, -2, 3, -4};
    std::string got = p.ToString();
    std::string expected = "-4x^3+3x^2-2x+1";
    EXPECT_EQ(got, expected) << "Negative coefficients: expected " << expected << ", got " << got;
}

TEST(PolynomialBoundaryTest, CoefficientOne) {
    Polynomial<ArraySequence, int> p{1, 1, 1};
    std::string got = p.ToString();
    std::string expected = "x^2+x+1";
    EXPECT_EQ(got, expected) << "Coefficient 1: expected " << expected << ", got " << got;
}
